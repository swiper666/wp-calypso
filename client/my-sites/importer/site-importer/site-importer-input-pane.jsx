...
class SiteImporterInputPane extends React.Component {
	...
	render() {
		...
		const error = importingError;
		const isLoading = this.state.loading || isImporting;
...
export default flow(
	connect(
		state => {
			const siteImporter = get( state, 'imports.siteImporter' );

			return {
				isImporting: siteImporter.isLoading,
				importingError: siteImporter.error,
				importData: siteImporter.importData,
				importStage: siteImporter.importStage,
				urlInputValue: siteImporter.urlInputValue,
			};
		},
		{
			recordTracksEvent,
